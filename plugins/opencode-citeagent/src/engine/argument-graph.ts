...
export class ArgumentGraphEngine {
  ...
  async queryClaims(args: Record<string, unknown>): Promise<{ claims: Record<string, unknown>[]; total: number }> {
    ...
    const claimsPath = sourceId
      ? path.join(this.corpusRoot, sourceId, `${sourceId}_claims.jsonl`)
      : null;

    const claims: Record<string, unknown>[] = [];

    if (claimsPath && fs.existsSync(claimsPath)) {
      const lines = fs.readFileSync(claimsPath, "utf-8").split("\n").filter(Boolean);
  ...
  async queryContradictions(args: Record<string, unknown>): Promise<{ contradictions: Record<string, unknown>[]; note: string }> {
    return {
      contradictions: [],
  ...
  async indexClaim(args: Record<string, unknown>): Promise<{ claim_id: string; status: string }> {
    ...
    const claimsPath = path.join(docDir, `${sourceId}_claims.jsonl`);
