...
export class CryptoEngine {
  ...
  private getSessionKey(sessionId: string): string {
    if (!this.sessionKeys.has(sessionId)) {
      this.sessionKeys.set(sessionId, crypto.createHash("sha256").update(`session:${sessionId}`).digest("hex"));
    }
    return this.sessionKeys.get(sessionId)!;
  }

  async sign(message: string, sessionId: string): Promise<{ signature: string; session_id: string }> {
    ...
    const trailPath = path.join(dir, `${sessionId}_trail.jsonl`);
    fs.appendFileSync(trailPath, JSON.stringify({
      action: "sign",
  ...
  async verify(message: string, signature: string, sessionId: string): Promise<{ valid: boolean; session_id: string }> {
    const key = this.getSessionKey(sessionId);
  ...
  async getAuditTrail(sessionId: string): Promise<{ entries: Record<string, unknown>[]; session_id: string }> {
    const trailPath = path.join(this.corpusRoot, ".crypto", `${sessionId}_trail.jsonl`);
